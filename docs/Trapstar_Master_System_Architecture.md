...
**File:** `Trapstar_Master_System_Architecture.md`  
**Project:** *Trapstar the Demo*  
**Purpose:** Master high-level system and technical-boundary reference  
**Status:** Architecture overview / living design truth / Phaser demo portability pass  
...
Content Data
  -> Run-Card Generator
    -> Portable Simulation State
      -> SEN / DPA / BASED Resolution
        -> State Transition
...
```text
The Phaser layer sends player intentions to the simulation and displays the resulting state.
It does not own the rules that determine those results.
```
...
A Phaser scene may send an action request such as `pressure_npc`, `enter_location`, or `spend_item`, but the simulation layer decides whether that action is valid, what it costs, what outcome occurs, and what state changes follow.
...
| **Presentation Adapter** | Simulation state, Phaser presentation | Converts player intentions into simulation requests and simulation results into display instructions. |
...
*Phaser 3.90 presents the seven-week demo, while portable data, stable state, simulation rules, reproducible tests, and standardized assets define the game beneath it and should survive any later production-engine migration.*
